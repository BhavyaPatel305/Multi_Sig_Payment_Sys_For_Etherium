...
contract MultiSig {
	...
	function pay (address from, uint amount, address to, uint256 pkx, uint256 pky, uint256 xx, uint xy, uint256 z, uint256 message) public{
    ...
    bytes memory m = abi.encodePacked(message);
            if (verify(z, CommonStructs.Point(pkx, pky), CommonStructs.Point(xx, xy), m)){
	...
	//This is taken right from schnorr.sol
	function verify(uint256 zBar, CommonStructs.Point memory pkBar, CommonStructs.Point memory xBar, bytes memory message) public returns (bool) {
